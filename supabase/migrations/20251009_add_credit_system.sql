...
-- Update existing starter businesses (includes trial users)
-- Note: Trial users are just starter tier with limited credits (50)
-- They get reset date far in future so credits don't reset until they upgrade
UPDATE businesses
SET
  monthly_credits = CASE
    WHEN subscription_status = 'trialing' THEN 50
    ELSE 500
  END,
  purchased_credits = 0,
  credits_used_this_month = 0,
  credits_reset_date = CASE
    WHEN subscription_status = 'trialing' THEN NOW() + INTERVAL '10 years'
