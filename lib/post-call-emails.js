/**
 * Post-Call Email Templates (Plain JS for webhook-server.js)
 *
 * Generates follow-up email HTML for leads qualified by Jordan (lead qualifier phone employee).
 * Mirror of src/lib/post-call-emails.ts for use in the Express webhook server.
 */
 ...
 * @param {string} params.callerEmail
 * @param {string} [params.callerCompany]
 * @param {'hot'|'warm'|'cold'} params.tier
 * @param {string} [params.interest]
 * @param {string} [params.reasoning]
 * @returns {{ subject: string, html: string, text: string } | null}
 */
function generatePostCallEmail(params) {
    switch (params.tier) {
        case 'hot':
...
function generateHotLeadEmail(params) {
    const firstName = (params.callerName || '').split(' ')[0] || 'there';
    const companyLine = params.callerCompany ? ` for ${params.callerCompany}` : '';
...
function generateWarmLeadEmail(params) {
    const firstName = (params.callerName || '').split(' ')[0] || 'there';
    const businessRef = params.callerCompany ? params.callerCompany : 'your business';
...
module.exports = { generatePostCallEmail };
