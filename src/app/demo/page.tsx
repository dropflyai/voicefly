...
function DemoPageContent() {
  ...
  const startCall = useCallback(async () => {
    ...
    if (!vapiKey) { setErrorMessage('Voice demo not configured. Please try again later.'); return }
    if (!industry.assistantId) { setErrorMessage('Demo assistant not configured for this industry.'); return }
    ...
    <div className="min-h-screen bg-surface font-[family-name:var(--font-inter)]">
      ...
      <header className="glass sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-xs bg-surface-high text-text-muted px-2 py-0.5 rounded-full">Demo</span>
          </Link>
          <Link
            href="/signup"
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="text-center mb-10">
          <p className="text-sm font-medium text-text-muted mb-2">Live Interactive Demo</p>
          <h1 className="text-3xl sm:text-4xl font-bold text-text-primary font-[family-name:var(--font-manrope)] tracking-tight mb-3">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium transition-all ${
                industryKey === ind.id
                  ...
                  : 'bg-surface-high text-text-secondary hover:bg-surface-highest hover:text-text-primary'
              }`}
            >
              <span>{ind.emoji}</span>
              {ind.label}
        ...
        {/* Main Demo Card */}
        <div className="bg-surface-low rounded-xl sonic-shadow overflow-hidden max-w-2xl mx-auto">
          {/* Persona Header */}
          <div className="bg-surface-med px-6 py-5">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-full bg-surface-high flex items-center justify-center text-2xl">
                {industry.emoji}
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="font-bold text-text-primary text-lg font-[family-name:var(--font-manrope)]">{industry.persona}</h2>
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-brand-primary/10 text-brand-light">
                    {industry.tagline}
                  </span>
                </div>
          ...
          <div className="px-6 py-5 min-h-[200px]">
              ...
              <div className="text-center py-6">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {industry.examples.map(example => (
                    <span key={example} className="text-xs bg-surface-high text-text-secondary px-3 py-1.5 rounded-full">
                      &ldquo;{example}&rdquo;
                    </span>
                  ))}
                </div>
            ...
            {status === 'connecting' && (
              <div className="flex flex-col items-center justify-center py-8 gap-3">
                ...
                <p className="text-sm text-text-muted">Connecting to {industry.persona}...</p>
              </div>
            )}
            ...
            {status === 'active' && (
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-text-muted">Live call with {industry.persona}</span>
                <span className="ml-auto text-xs font-mono text-text-muted">{formatTime(callDuration)}</span>
              </div>
            )}
            ...
            {errorMessage && (
              <div className="mt-3 p-3 bg-[#93000a]/10 rounded-lg">
                <p className="text-sm text-[#ffb4ab]">{errorMessage}</p>
              </div>
            )}
          </div>
          ...
          <div className="px-6 pb-5">
            {status === 'idle' || status === 'error' ? (
              <button
                onClick={startCall}
                className="w-full py-3.5 rounded-lg text-brand-on font-semibold text-sm transition-all bg-brand-primary hover:bg-[#0060d0] flex items-center justify-center gap-2"
              >
                <PhoneIcon />
                Talk to {industry.persona}
              </button>
            ) : status === 'connecting' ? (
              <div className="w-full py-3.5 rounded-lg bg-surface-high text-text-muted font-medium text-sm flex items-center justify-center gap-2">
                <div className="w-4 h-4 border-2 border-surface-highest border-t-brand-primary rounded-full animate-spin" />
                Connecting...
              </div>
            ) : status === 'active' ? (
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 py-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 ${
                    isMuted
                      ? 'bg-[#ca4e00]/20 text-accent'
                      : 'bg-surface-high text-text-secondary hover:bg-surface-highest'
                  }`}
                >
                ...
                <button
                  onClick={endCall}
                  className="flex-1 py-3 rounded-lg bg-[#93000a] text-[#ffb4ab] font-medium text-sm hover:bg-[#b01215] transition-colors flex items-center justify-center gap-2"
                >
                  <PhoneOffIcon />
                  End Call
                </button>
          ...
          <div className="mt-6 max-w-2xl mx-auto bg-surface-med rounded-xl p-6 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <button
                ...
                className="px-6 py-3 bg-surface-high text-text-secondary rounded-lg font-medium text-sm hover:bg-surface-highest transition-colors"
              >
                Try another demo
              </button>
            </div>
        ...
        {/* Industry grid */}
        {!callEnded && (
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-2 max-w-2xl mx-auto">
            {Object.values(INDUSTRIES)
              ...
              .map(ind => (
                <button
                  key={ind.id}
                  onClick={() => handleIndustryChange(ind.id as IndustryKey)}
                  className="bg-surface-low hover:bg-surface-med rounded-lg p-4 text-left transition-all group"
                >
                  <div className="text-xl mb-1">{ind.emoji}</div>
                  <div className="text-sm font-medium text-text-primary">{ind.persona}</div>
                  <div className="text-xs text-text-muted">{ind.label}</div>
                </button>
              ))}
          </div>
        )}
